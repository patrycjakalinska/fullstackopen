const {
  totalLikes,
  favouriteBlog,
  dummy,
  mostLikes,
  mostBlogs,
} = require('../utils/list_helper')

const listWithOneBlog = [
  {
    _id: '5a422aa71b54a676234d17f8',
    title: 'Go To Statement Considered Harmful',
    author: 'Edsger W. Dijkstra',
    url: 'http://www.u.arizona.edu/~rubinson/copyright_violations/Go_To_Considered_Harmful.html',
    likes: 5,
    __v: 0,
  },
]
const initialBlogs = [
  {
    _id: '5a422a851b54a676234d17f7',
    title: 'React patterns',
    author: 'Michael Chan',
    url: 'https://reactpatterns.com/',
    likes: 7,
    __v: 0,
  },
  {
    _id: '5a422aa71b54a676234d17f8',
    title: 'Go To Statement Considered Harmful',
    author: 'Edsger W. Dijkstra',
    url: 'http://www.u.arizona.edu/~rubinson/copyright_violations/Go_To_Considered_Harmful.html',
    likes: 5,
    __v: 0,
  },
  {
    _id: '5a422b3a1b54a676234d17f9',
    title: 'Canonical string reduction',
    author: 'Edsger W. Dijkstra',
    url: 'http://www.cs.utexas.edu/~EWD/transcriptions/EWD08xx/EWD808.html',
    likes: 12,
    __v: 0,
  },
  {
    _id: '5a422b891b54a676234d17fa',
    title: 'First class tests',
    author: 'Robert C. Martin',
    url: 'http://blog.cleancoder.com/uncle-bob/2017/05/05/TestDefinitions.htmll',
    likes: 10,
    __v: 0,
  },
  {
    _id: '5a422ba71b54a676234d17fb',
    title: 'TDD harms architecture',
    author: 'Robert C. Martin',
    url: 'http://blog.cleancoder.com/uncle-bob/2017/03/03/TDD-Harms-Architecture.html',
    likes: 30,
    __v: 0,
  },
  {
    _id: '5a422bc61b54a676234d17fc',
    title: 'Type wars',
    author: 'Robert C. Martin',
    url: 'http://blog.cleancoder.com/uncle-bob/2016/05/01/TypeWars.html',
    likes: 2,
    __v: 0,
  },
]

test('dummy returns 1', () => {
  const blogs = []

  const result = dummy(blogs)
  expect(result).toBe(1)
})

describe('total likes', () => {
  test('of empty list is zero', () => {
    expect(totalLikes([])).toBe(0)
  })

  test('when list has only one blog equals the likes of that', () => {
    const res = totalLikes(listWithOneBlog)

    expect(res).toBe(5)
  })

  test('of a bigger list is calculated right', () => {
    const res = totalLikes(initialBlogs)

    expect(res).toBe(66)
  })
})

describe('favourite blog', () => {
  test('of empty list equals zero', () => {
    expect(favouriteBlog([])).toBe(0)
  })

  test('of list with one blog equals to its likes', () => {
    expect(favouriteBlog(listWithOneBlog).likes).toBe(5)
  })

  test('of a bigger list is selected right', () => {
    expect(favouriteBlog(initialBlogs)).toEqual(initialBlogs[4])
  })
})

describe('most likes', () => {
  test('of empty list is 0', () => {
    expect(mostLikes([])).toBe(0)
  })
  test('when only one element, it equals the likes of this element', () => {
    expect(mostLikes(listWithOneBlog)).toEqual({
      author: 'Edsger W. Dijkstra',
      likes: 5,
    })
  })
  test('of a bigger list is correct', () => {
    expect(mostLikes(initialBlogs)).toEqual({
      author: 'Robert C. Martin',
      likes: 42,
    })
  })
})

describe('most blogs', () => {
  test('of empty list is 0', () => {
    expect(mostBlogs([])).toBe(0)
  })
  test('when only one element is correct', () => {
    expect(mostBlogs(listWithOneBlog)).toEqual({
      author: 'Edsger W. Dijkstra',
      blogs: 1,
    })
  })
  test('of a bigger list is correct', () => {
    expect(mostBlogs(initialBlogs)).toEqual({
      author: 'Robert C. Martin',
      blogs: 3,
    })
  })
})
